...
class CustomSHAPexplainer():
	def __init__(self):
		...
		self.model = Model(Config.model, Config.bias, Config.temperature)
		self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

		checkpoint = torch.load("checkpoint/29249.pth", map_location=self.device, weights_only=True)
		self.model.eval()
	...
	def shap_eval(self, sentence, steps, nsamples=300):
		...
		decoded_memory = [self.tokenizer.decode([token], skip_special_tokens=False) for token in
						  memory_ids[self.memory.gate_mask == 0]]
